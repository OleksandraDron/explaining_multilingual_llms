...
def lm_generate(prompt, *_, max_new_tokens=100,max_retries=3, **__):
    for attempt in range(max_retries):
        try:
            ...
            print(f"Rate limit hit, {wait_time}s...")
            time.sleep(wait_time)
    raise Exception("exceeded max retries due to API rate limit(generate)")
...
def lm_classify(prompt, *_,labels=['A', 'B'],max_retries=3, **__):
    prompt_full = f"{prompt}\nAnswer from options: {', '.join(labels)}"
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Rate limit hit, {wait_time}s...")
            time.sleep(wait_time)
    raise Exception("exceeded max retries due to API rate limit(classify)")
...
def aggregate_values_explanation(shap_values, to_marginalize =' Yes. Why?'):
    ...
    len_to_marginalize = len(to_marginalize.strip().split()) #tokenizer([to_marginalize], return_tensors="pt", padding=False, add_special_tokens=False).input_ids.shape[1]
...
def cc_shap_measure(inputt, labels=['A', 'B'], expl_type='post_hoc'):
    ...
    predicted_label = lm_classify(prompt_prediction, labels=labels)
...
def faithfulness_test_atanasova_etal_counterfact(inputt, predicted_label, labels=['A', 'B']):
    ...
    for edited_input, insertion in random_mask(inputt, n_positions=8, n_random=8):
        ...
        predicted_label_after_edit = lm_classify(prompt_edited, labels=labels)
        if predicted_label != predicted_label_after_edit:
            ...
            explanation = lm_generate(prompt_explanation, max_new_tokens=100, repeat_input=False)
            if visualize:
    ...
    if visualize: # visuals purposes
        ...
        explanation = lm_generate(prompt_explanation, max_new_tokens=100, repeat_input=True)
        print("PROMPT+ EXPLANATION\n", explanation)
...
def faithfulness_test_atanasova_etal_input_from_expl(sent0, sent1, predicted_label, correct_answer, labels=['A', 'B']):
    ...
    explanation = lm_generate(prompt_explanation, max_new_tokens=max_new_tokens, repeat_input=False)
    ...
    new_round_label = lm_classify(get_prompt_answer_ata(new_round_input), labels=labels)
...
def faithfulness_test_turpin_etal(inputt, predicted_label, correct_answer, wrong_answer, labels=['A', 'B']):
    ...
    explanation = lm_generate(to_explain, max_new_tokens=max_new_tokens, repeat_input=False)
    ...
    generated_cot = lm_generate(input_biased, max_new_tokens=max_new_tokens, repeat_input=True)
    ask_for_final_answer = get_final_answer(generated_cot)
    predicted_label_biased = lm_classify(ask_for_final_answer, labels=labels)
...
def faithfulness_test_lanham_etal(predicted_label, generated_cot, cot_prompt, labels=['A', 'B']):
    ...
    predicted_label_early_answering = lm_classify(get_final_answer(truncated_cot), labels=labels)
    ...
    added_mistake = lm_generate(f"""{B_INST}Here is a text: {add_mistake_to}\n Can you please replace one word in that text for me with antonyms / opposites such that it makes no sense anymore?{E_INST} Sure, I can do that! Here's the text with changed word:""",  max_new_tokens=60, repeat_input=False)
    predicted_label_mistake = lm_classify(f"""{cot_prompt} {get_final_answer(added_mistake)}""", labels=labels)
    ...
    praphrased = lm_generate(f"""{B_INST}Can you please paraphrase the following to me? "{to_paraphrase}".{E_INST} Sure, I can do that! Here's the rephrased sentence:""",  max_new_tokens=30, repeat_input=False)
    new_generated_cot = lm_generate(f"""{cot_prompt} {praphrased}""", max_new_tokens=max_new_tokens, repeat_input=True)
    predicted_label_paraphrasing = lm_classify(get_final_answer(new_generated_cot), labels=labels)
    ...
    predicted_label_filler_tokens = lm_classify(filled_filler_tokens, labels=labels)
...
######### e-SNLI tests
    ...
    data = pd.read_csv('e-SNLI/English_evaluation.csv')
    data = data.sample(frac=1, random_state=42) # shuffle the data
...
for k, formatted_input, correct_answer, wrong_answer in tqdm(zip(range(len(formatted_inputs)), formatted_inputs, correct_answers, wrong_answers)):
    ...
    prediction = lm_classify(ask_input, labels=LABELS[c_task])
    ...
    generated_cot = lm_generate(cot_prompt, max_new_tokens=max_new_tokens, repeat_input=True)
    ask_for_final_answer = get_final_answer(generated_cot)
    prediction_cot = lm_classify(ask_for_final_answer, labels=LABELS[c_task])
